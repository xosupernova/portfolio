...
interface HeaderProps { forceDesktop?: boolean }
		...
		<header className="sticky top-0 z-30 w-full backdrop-blur supports-[backdrop-filter]:bg-white/40 dark:supports-[backdrop-filter]:bg-black/30 bg-white/70 dark:bg-black/40 border-b border-black/5 dark:border-white/5">
			<div className="container mx-auto grid grid-cols-[auto_1fr_auto] items-center px-4 py-3">
				<h1 className="text-xl font-bold flex items-center space-x-2 select-none">
					<Icon icon="streamline-flex:music-equalizer-solid" className="inline-block align-middle size-9" />
					<span>{env.VITE_APP_TITLE}</span>
				</h1>
				<nav className={forceDesktop ? 'flex justify-center' : 'hidden lg:flex justify-center'} aria-label="Main navigation">
					<NavigationMenu>
						<NavigationMenuList>
							{NAV_ITEMS.map(item => (
					...
					<DropdownMenu>
						<Tooltip>
							<TooltipTrigger asChild>
								<DropdownMenuTrigger data-testid="theme-trigger" aria-label="Theme toggle" className={(forceDesktop ? 'inline-flex' : 'hidden lg:inline-flex') + ' h-9 w-9 items-center justify-center rounded-full border bg-white/60 dark:bg-black/40 backdrop-blur hover:bg-gray-100 dark:hover:bg-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500'}>
									{!mounted ? (
										<Icon icon="line-md:sun-rising-filled-loop" className="size-5 text-yellow-500" />
									) : isDark ? (
						...
						<DropdownMenuContent align="end">
							<DropdownMenuItem onSelect={() => setTheme('light')}>
								<Icon icon="line-md:sun-rising-filled-loop" className="mr-2 size-4 text-yellow-500" /> Light
							</DropdownMenuItem>
							<DropdownMenuItem onSelect={() => setTheme('dark')}>
								<Icon icon="line-md:moon" className="mr-2 size-4" /> Dark
							</DropdownMenuItem>
							<DropdownMenuSeparator />
							<DropdownMenuItem disabled className="opacity-50">System (soon)</DropdownMenuItem>
						</DropdownMenuContent>
					</DropdownMenu>
					<Dialog>
						<DialogTrigger className="lg:hidden inline-flex items-center justify-center rounded-md p-2 text-gray-700 dark:text-gray-200 hover:bg-black/5 dark:hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500" aria-label="Open menu">
