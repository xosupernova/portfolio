...
function renderWithRouter(node: React.ReactElement) {
  const rootRoute = new RootRoute({ component: () => <TooltipProvider delayDuration={0}>{node}</TooltipProvider> });
  const indexRoute = new Route({ getParentRoute: () => rootRoute, path: '/', component: () => null });
  const aboutRoute = new Route({ getParentRoute: () => rootRoute, path: 'about', component: () => null });
  const projectsRoute = new Route({ getParentRoute: () => rootRoute, path: 'projects', component: () => null });
  const contactRoute = new Route({ getParentRoute: () => rootRoute, path: 'contact', component: () => null });
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <DialogDescription>This is a sample dialog used for accessibility tests.</DialogDescription>
  ...
  return (
    <Dialog>
      ...
      <DialogContent variant="right">
        ...
        <a href="#a" data-testid="sheet-link">Link</a>
      </DialogContent>
    </Dialog>
  );
}
...
describe('Accessibility', () => {
  it('dialog closes with Escape and returns focus', async () => {
    ...
    await waitFor(() => expect(document.activeElement).toBe(screen.getByTestId('open-dialog')));
  });
