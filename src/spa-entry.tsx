...
const enableReplay =
	(import.meta.env.VITE_ENABLE_SENTRY_REPLAY || 'false') === 'true';
...
if (enableSentry && dsn) {
	...
	const init = () => {
		...
		const baseSessionReplayRate = enableReplay
			? env === 'production'
				? 0.02
				: 0.1
			: 0.0;
		Sentry.init({
			...
			beforeSend(event, hint) {
				...
				if (/NetworkError when attempting to fetch resource/i.test(msg))
					return null;
				return event;
			},
