export const onRequest: PagesFunction = async (ctx) => {
  ...
  // Derive slug from first path segment; root -> index
  ...
  // Strip conflicting title/description and OG/Twitter tags
  const patterns: RegExp[] = [
    /<title[^>]*>[\s\S]*?<\/title>\s*/gi,
    /<meta[^>]+name=["']description["'][^>]*>\s*/gi,
    /<meta[^>]+property=["']og:(?:title|description|image|image:secure_url|type|url)["'][^>]*>\s*/gi,
    /<meta[^>]+name=["']twitter:(?:card|title|description|image|image:src|url|domain)["'][^>]*>\s*/gi,
